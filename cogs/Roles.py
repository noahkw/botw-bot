...
class Roles(CustomCog, AinitMixin):
    ...
    async def _clear_role(self, guild: int, member: int, role: int):
        ...
        if guild_:
            ...
            if member_ and role_:
                ...
                logger.info(
                    f"Automatically unassigned {role_} from {member_} in {guild_}."
                )
        ...
        try:
            if role not in member.roles:
                ...
                if clear_after := row["clear_after"]:
                    ...
                    self.scheduler.schedule(
                        self._clear_role(member.guild.id, member.id, role.id), when
                    )
                    query_schedule_clear = """INSERT INTO role_clears
                                              VALUES ($1, $2, $3, $4);"""
                    await self.bot.pool.execute(
                        query_schedule_clear, role.id, member.id, when, member.guild.id
                    )
            else:
            ...
            async with connection.transaction():
                ...
                try:
                    await connection.execute(
                        query_role, ctx.guild.id, role.id, clear_after
                    )
                ...
                for alias in aliases:
                    ...
                    try:
                        await connection.execute(
                            query_alias, role.id, ctx.guild.id, alias
                        )
                    except asyncpg.exceptions.UniqueViolationError:
                        raise commands.BadArgument(
    ...
    async def info(self, ctx, *, alias):
        ...
        await ctx.send(
            f'Role: {role.mention}, clears after: {row["clear_after"] or "∞"} hours.'
