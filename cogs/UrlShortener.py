...
class UrlShortener(commands.Cog):
    ...
    async def shorten_url(self, url):
        payload = {"domain": self.domain, "long_url": url}

        async with self.session.post(
            self.SHORTEN_ENDPOINT, json=payload, auth=self.auth
        ) as response:
    ...
    async def shorten(self, ctx, *urls):
        ...
        for url in urls:
            try:
                ...
                await ctx.send(
                    f"Could not shorten `{url}`. Make sure that it is a valid URL."
                )

        if len(shortened_urls) > 0:
            await ctx.send("Your shortened URL(s):")
            await ctx.send("```" + "\n".join(shortened_urls) + "```")
