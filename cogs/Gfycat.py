...
class Gfycat(CustomCog, AinitMixin):
    ...
    @gfycat.command(brief='Display the search in a nice-to-copy-paste format', hidden=True)
    async def format(self, ctx, *, query):
        ...
        if len(gfys) == 0:
            raise commands.BadArgument('Could not find any gfys that meet the given criteria.')
    ...
    async def plot(self, ctx, user_id, filter_outliers: typing.Optional[bool] = False,
                   start: int = None, end: int = None):
        with ctx.typing():
            gfys = await self.gfypy.get_user_feed(user_id=user_id, limit=-1)

            data = [(int(gfy.create_date.strftime('%y%m%d')), gfy.views) for gfy in gfys]
            data = pd.DataFrame(data, columns=['date', 'views'])

            if filter_outliers:
                data = data[data.views <= data.views.quantile(.85)]
            ...
            await ctx.send(f'{ctx.author.mention}, here\'s the diagram. Average views per gfy'
