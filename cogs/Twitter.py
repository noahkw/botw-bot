...
class Twitter(CustomCog, AinitMixin):
    ...
    def __init__(self, bot):
        ...
        self.keys["consumer_secret"] = self.bot.config["cogs"]["twitter"][
            "consumer_secret"
        ]
        self.keys["access_token"] = self.bot.config["cogs"]["twitter"]["access_token"]
        self.keys["access_token_secret"] = self.bot.config["cogs"]["twitter"][
            "access_token_secret"
        ]
        ...
        async with self.feed as stream:
            ...
            async for tweet in self.stream_thing:
                if on_tweet_with_media(tweet):
                    logger.info(
                        f"Initiating stream processing for: @{tweet.user.id_str} - {tweet.id_str}"
                    )
    ...
    async def stats(self, ctx):
        accts_followed = len(await self.generate_accounts())
        time_start = time.time()
        await self.client.api.statuses.show.get(
            id=1230927030163628032, include_entities=True, tweet_mode="extended"
        )
        time_end = time.time()
        await ctx.send(
            f"Currently following - {accts_followed} accounts\nTwitter API Latency - {(time_end-time_start)*1000}ms"
        )
    ...
    async def enable(self, ctx, channel: discord.TextChannel):
        ...
        async with self.bot.Session() as session:
            ...
            if server_settings:
                ...
                logger.info(
                    f"Guild {ctx.guild.id} default channel updated to {channel.id}"
                )
            else:
                server_settings = twt_settings(
                    _guild=ctx.guild.id, default_channel=channel.id
                )
                session.add(server_settings)
                logger.info(
                    f"Guild {ctx.guild.id} enabled twitter with default channel {channel.id}"
                )
        ...
        if tweet_ids:
            ...
            async with self.bot.Session() as session:
                for tweet_id in tweet_ids:
                    ...
                    if tweet:
                        logger.info(
                            f"Manual processing for: @{tweet.user.id_str} - {tweet.id_str} in Guild {ctx.guild.id}"
                        )
    ...
    async def add_hashtag(
        ...
        async with self.bot.Session() as session:
            ...
            if twitter_filter:
                ...
                logger.info(
                    f"Guild {ctx.guild.id} updated hastag {tag_conditioned} to channel {channel}"
                )
                await ctx.send(f"#{tag_conditioned} channel updated to {channel}")
            else:
                logger.info(
                    f"Guild {ctx.guild.id} added hastag {tag_conditioned} to channel {channel}"
                )

            twitter_filter = twt_filters(
    ...
    async def add_account(self, ctx, account: commands.clean_content):
        ...
        """
        account_conditioned = account.split("@")[
            -1
        ...
        if account_id:
            ...
            async with self.bot.Session() as session:
                accounts_list = await db.get_twitter_accounts(
                    session, account_id=account_id, _guild=ctx.guild.id
                )
                ...
                if accounts_list:  # check if already followed
                    ...
                    new_account = twt_accounts(
                        _guild=ctx.guild.id, account_id=account_id
                    )
    ...
    async def get_channels(self, servers, tags, session):
        ...
        for server in servers_channels_raw:
            servers_dict[server._guild].add(
                server._channel
            )  # just one of each, so we use a set, since multiple hashtags can map to same channel
    ...
    async def create_post(self, tweet, is_stream=True):
        ...
        tweet_date = (tweet_date + UTC_OFFSET).strftime("%y%m%d")

        # date_re = re.compile("(\s+|^)(\d{6}|\d{8})\s+")
        # date_result = date_re.match(tweet.text)
        # event_dates_result = re.search(event_date_re, tweet.text).group(2)
        # event_dates = [date.group(2) for date in event_dates_result]
        event_date = re.search(self.EVENT_DATE_REGEX, tweet.text).group(2)
